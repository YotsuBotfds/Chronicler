...
The `adherents` dict serves double duty: keys = who has adopted, values = variant counter for schism tracking. Variant counter starts at `hash(civ_name + movement_id) % SEEDED_OFFSET_RANGE`, increments by 1 every `VARIANT_DRIFT_INTERVAL` turns.
...
**Normalization:** Both culture and stability are divided by `STAT_MAX` (100 post-P1), producing values in [0, 1]. This formula survives P1 scale migration without retuning.
...
movement = Movement(
    ...
    adherents={winner.name: hash(winner.name + movement.id) % SEEDED_OFFSET_RANGE},
)
world.movements.append(movement)
world.next_movement_id += 1
```

Origin civ auto-adopts with seeded starting offset. Generate `NamedEvent(event_type="movement_emergence", importance=6)`.
...
roll = deterministic_roll(seed, turn, movement.id, target.name)  # 0-99
if roll < adoption_probability and target.name not in movement.adherents:
    movement.adherents[target.name] = hash(target.name + movement.id) % SEEDED_OFFSET_RANGE
...
```python
# Auto-fires when a region is targeted by INVEST_CULTURE
...
| `movement_schism` | 7 | `_detect_schisms` | Check events_timeline for same movement + actor pair |
...
| `cultural_hegemony` | 9 | `check_cultural_victories` | Check events_timeline for same actor |
| `universal_enlightenment` | 10 | `check_cultural_victories` | Check events_timeline for movement.id in description |
