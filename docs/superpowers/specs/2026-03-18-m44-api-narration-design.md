...
2. Simulation loop: per-turn narrator callback is `lambda w, e: ""` (noop). No per-turn prose, no API calls during simulation. **Reflections are also skipped** — the `should_reflect` block is gated off entirely in API mode (not fed a dummy client, which would produce `"DEVELOP"` text that leaks into `compile_chronicle()` via `era_reflections`). Memory streams still accumulate; only the LLM-generated reflection prose is skipped. `era_reflections` stays empty, `compile_chronicle()` inserts no era headers. With `reflection_interval=10` and 500 turns, reflections would otherwise produce ~200 API calls during the loop (50 rounds x N civs).
...
| 14 | Skip reflections in API mode | `generate_reflection()` uses `_narr` directly, not the engine's narrator callback. With `reflection_interval=10`, that's ~200 unaccounted API calls. Gate off the `should_reflect` block entirely — don't use `_DummyClient` (its `"DEVELOP"` return leaks into `compile_chronicle()` via `era_reflections`). Memory streams still accumulate; only LLM prose is skipped. |
