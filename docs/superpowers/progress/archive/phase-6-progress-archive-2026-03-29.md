...
- **Next implementation target:** Start M58a from the clean `main` line and keep the accepted M54 controls, the final M55 gate artifact, and the tuned M57b gate artifacts immutable for future comparisons.
...
- **M58a prep (2026-03-28):** Added `docs/superpowers/plans/2026-03-28-m58a-pre-spec-handoff.md` to capture the live post-M57 baseline for `M58a: Merchant Mobility`, including route-selection seams, reservation/transit architecture options, determinism constraints, and the concrete user decisions to lock before spec freeze.
