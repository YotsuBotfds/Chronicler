...
fn bench_cache_efficiency(c: &mut Criterion) {
    ...
    group.bench_function("packed_10k", |b| {
        b.iter_batched(setup_packed, |(mut pool, regions, signals)| {
            ...
            tick_agents(black_box(&mut pool), black_box(&regions), black_box(&signals), seed, 0, &mut percentiles, &mut Vec::new(), &[], &mut chronicler_agents::spatial::SpatialDiagnostics::default(), &[], None);
