...
validation JSON artifacts are written with strict JSON serialization and atomic same-directory replacement: `NaN`/`Infinity` and duplicate JSON keys are rejected rather than preserved as misleading evidence, and interrupted writes should not leave truncated report artifacts.
