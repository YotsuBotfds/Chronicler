"""Validation IO helpers and oracle implementations restored from the M53b monolith.

This module intentionally contains the recovered implementation so the
``chronicler.validate`` facade can import again while the split modules remain
small and auditable.
"""
...
class ValidationDependencyError(RuntimeError):
    """Required optional dependency is missing for a requested oracle."""
...
def _iter_seed_dirs(batch_dir: Path) -> list[Path]:
    if batch_dir.is_file():
        return [batch_dir.parent]
    return sorted(path.parent for path in batch_dir.rglob(BUNDLE_FILENAME))


def _requires_pyarrow_for_selected_inputs(batch_dir: Path, selected_oracles: set[str] | None) -> str | None:
    if selected_oracles is None or _has_pyarrow():
        return None
    selected = set(selected_oracles)
    seed_dirs = _iter_seed_dirs(batch_dir)
    if "regression" in selected and any((seed_dir / "agent_events.arrow").exists() for seed_dir in seed_dirs):
        return "regression oracle requires pyarrow"
    if "needs" in selected and any(
        (seed_dir / "validation_needs.arrow").exists()
        and (seed_dir / "agent_events.arrow").exists()
        for seed_dir in seed_dirs
    ):
        return "needs oracle requires pyarrow"
    if "cohort" in selected and any(
        (seed_dir / "validation_relationships.arrow").exists()
        and (seed_dir / "validation_needs.arrow").exists()
        and (seed_dir / "agent_events.arrow").exists()
        for seed_dir in seed_dirs
    ):
        return "cohort oracle requires pyarrow"
    return None
...
def load_bundles(batch_dir: Path) -> list[tuple[Path, dict]]:
    ...
    if not batch_dir.exists():
        raise ValidationRequestError(f"batch path does not exist: {batch_dir}")
    if batch_dir.is_file():
...
def load_seed_runs(batch_dir: Path, selected_oracles: set[str] | None = None) -> list[dict]:
    """Load exported bundle + sidecar data for each seed in a batch."""
    if not batch_dir.exists():
        raise ValidationRequestError(f"batch path does not exist: {batch_dir}")
    dependency_reason = _requires_pyarrow_for_selected_inputs(batch_dir, selected_oracles)
    if dependency_reason is not None:
        raise ValidationDependencyError(dependency_reason)
    runs: list[dict] = []
...
SEED_RUN_ORACLE_NAMES = ORACLE_RUNNER_NAMES - {"determinism"}


def _normalize_requested_oracles(oracles: list[str]) -> set[str]:
    selected = set(oracles)
    if "all" in selected:
        return set(ORACLE_RUNNER_NAMES)
    unknown = sorted(selected - ORACLE_RUNNER_NAMES)
    if unknown:
...
def run_oracles(batch_dir: Path, oracles: list[str]) -> dict:
    ...
    def ensure_seed_runs() -> list[dict]:
        ...
        if seed_runs is None:
            seed_runs = load_seed_runs(batch_dir, selected_oracles=selected & SEED_RUN_ORACLE_NAMES)
        return seed_runs

    for oracle_name in sorted(selected):
        try:
            if oracle_name == "determinism":
                results[oracle_name] = run_determinism_gate(batch_dir)
            elif oracle_name == "community":
                results[oracle_name] = run_community_oracle(ensure_seed_runs())
            elif oracle_name == "needs":
                results[oracle_name] = run_needs_oracle(ensure_seed_runs())
            elif oracle_name == "era":
                results[oracle_name] = run_era_oracle(ensure_seed_runs())
            elif oracle_name == "cohort":
                results[oracle_name] = run_cohort_oracle(ensure_seed_runs())
            elif oracle_name == "artifacts":
                results[oracle_name] = run_artifact_oracle(ensure_seed_runs())
            elif oracle_name == "arcs":
                results[oracle_name] = run_arc_oracle(ensure_seed_runs())
            elif oracle_name == "regression":
                results[oracle_name] = run_regression_summary(ensure_seed_runs())
...
__all__ = ['SCRUB_KEYS', 'BUNDLE_FILENAME', 'EVENT_NAME_TO_CODE', 'ValidationRequestError', 'ValidationDependencyError', '_has_pyarrow', '_iter_seed_dirs', '_requires_pyarrow_for_selected_inputs', 'scrubbed_equal', 'load_bundles', 'required_seed_count', '_read_json', '_snapshot_turns', '_choose_snapshot_turn', '_load_graph_snapshot', '_load_needs_snapshot', '_read_arrow_columns', '_load_canonical_graph_snapshot', '_load_canonical_needs_snapshot', '_available_turns_from_columns', '_sample_turns', '_columns_for_turn', '_graph_snapshot_from_columns', '_needs_snapshot_from_columns', '_load_validation_summary', '_load_validation_community_summary', '_load_agent_events', '_world_population_series', '_civ_trajectories', 'load_seed_runs', '_total_turns', '_iter_graph_snapshots', '_iter_needs_snapshots', '_iter_joint_sample_turns', '_filter_events_window', '_mean', '_cohen_d', '_collapse_nearby_points', '_trajectory_metric_series', '_trajectory_composite_series', '_smoothed_series', '_significant_slope_pattern', '_agent_count_by_turn', '_final_ginis_from_validation_summary', 'detect_communities', 'compute_needs_diversity', '_needs_candidate_priority', 'detect_inflection_points', 'compute_cohort_distinctiveness', '_cohort_candidate_priority', 'check_artifact_lifecycle', '_legacy_classify_civ_arc', '_classify_series_by_thirds', 'classify_civ_arc', 'run_determinism_gate', 'run_community_oracle', 'run_needs_oracle', 'run_era_oracle', 'run_cohort_oracle', 'run_artifact_oracle', 'run_arc_oracle', 'run_regression_summary', 'run_oracles', 'main']
